...
bool CActorInstance::__IsLeftHandWeapon(DWORD type)
{
	if (CItemData::WEAPON_DAGGER == type || (CItemData::WEAPON_FAN == type && __IsMountingHorse()))
		...
		return true;
	else if (CItemData::WEAPON_POLE == type)
		return false;
	else
		return false;
}
...
DWORD CActorInstance::AttachEffectByID(DWORD dwParentPartIndex, const char * c_pszBoneName, DWORD dwEffectID, const D3DXVECTOR3 * c_pv3Position, DWORD * dwSkillColor)
#else
...
#ifdef ENABLE_SKILL_COLOR_SYSTEM
	rkEftMgr.CreateEffectInstance(ae.dwEffectIndex, dwEffectID, dwSkillColor);
#else
...
void CActorInstance::HideAllAttachingEffect2()
{
	...
	{
		TAttachingEffect & rkAttEft = (*i);
		for ( int dwEID = 286; dwEID <= 419; ++dwEID)
		{
			if (rkAttEft.dwEffectIndex == dwEID)
			{
				CEffectManager::Instance().DestroyEffectInstance2(dwEID);
				//CEffectManager::Instance().HideEffect();
			}
		}
		++i;
	...
	for(it = m_AttachingEffectList.begin(); it!= m_AttachingEffectList.end();++it)
	{
		...
		while (dwEID != 419)
		{
			if (rkAttEft.dwEffectIndex == dwEID)
			{
				CEffectManager::Instance().SelectEffectInstance(dwEID);
				CEffectManager::Instance().HideEffect();
			}
			++dwEID;
		}
