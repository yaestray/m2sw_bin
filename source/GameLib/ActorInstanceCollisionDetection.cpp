...
* The first value is the minimum value and the second value is the maximum value of pet vnum (mob_proto) - change 34051 with your max vnum of pet */	
	...
	int pListMount[2] = {20110, 20299};
/************
* You can add whatever you like vnum of npc or monster (mob_proto) */
	int pListGlobal[] = {9001, 9002, 9003, 9004, 9005, 9006, 20011, 20091, 20092, 20093, 20094, 20095, 30000, 30001, 30002, 30003, 30004, 30005, 30006, 30007,20101, 20102, 20103, 20104, 20105, 20106, 20107, 20108, 20109};
/************
* Location name of the map where the event takes place ox */		
	...
	{	
		if (0 <= rVictim.GetRace() && rVictim.GetRace() <= 9) // Check if the victim through which pass over a player (change 7 with 8 if u have wolfman)
	...
	{
		if (rVictim.GetRace() == pListGlobal[i] || pListPet[0] <= rVictim.GetRace() && rVictim.GetRace() <= pListPet[1] || pListMount[0] <= rVictim.GetRace() && rVictim.GetRace() <= pListMount[1]) // Verify that the victim is npc vnum listed above, or if a pet.
