...
/*bool CItemManager::LoadItemNames(const char* c_szFileName)
{
	...
	for (DWORD i = 0; i < kTextFileLoader.GetLineCount(); ++i)
	{
		...
		const std::string& c_rstTransName = kTokenVector[1];
		TItemMap::iterator f = m_ItemMap.find (dwVnum);
		if (m_ItemMap.end() == f)
		{
			continue;
		...
		pkItemDataFind->SetTranslatedName (c_rstTransName);
	}
	return true;
}*/
...
{	
	...
	uint32_t counterItemWithoutIcons = 0;
		...
		if (m_ItemMap.end() == f)
		{
			...
			if (CResourceManager::Instance().IsFileExist(szName) == false)
			{
				...
				if (CResourceManager::Instance().IsFileExist(szName) == false)
				{
					...
					#endif
					++counterItemWithoutIcons;
					_snprintf(szName, sizeof(szName), "icon/item/noexist.tga");
				}
			}
	...
	Tracenf("ItemManager : There are %u items without icons.", counterItemWithoutIcons);
