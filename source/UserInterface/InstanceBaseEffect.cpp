...
void CInstanceBase::AttachTextTail()
{
	...
	if (IsPC())
		UpdateTextTailCountryFlag(m_szCountry);
}

void CInstanceBase::UpdateTextTailCountryFlag(const char* c_szCountry)
{
	if (m_dwLevel)
		CPythonTextTail::Instance().AttachCountryFlag(GetVirtualID(), c_szCountry);
}
...
void CInstanceBase::UpdateTextTailLevel(DWORD level)
{
	...
	/*CItemData * pItemData;
	if (IsGameMaster())
		if (CItemManager::Instance().GetItemDataPointer(CPythonPlayer::Instance().GetItemIndex(TItemPos(INVENTORY, c_Costume_Slot_Vip)), &pItemData))
			sprintf(szText, "[Premium] Lv %d ", level);
		else
			sprintf(szText, "Lv %d ", level);*/
...
void CInstanceBase::RefreshTextTail()
{
	...
	/*CItemData * pItemData;
	if (CItemManager::Instance().GetItemDataPointer(CPythonPlayer::Instance().GetItemIndex(TItemPos(INVENTORY, c_Costume_Slot_Vip)), &pItemData)) {
		CPythonNetworkStream::Instance().SendChatPacket("/prestige_title_name prestige_1");
	}
	else {
		CPythonNetworkStream::Instance().SendChatPacket("/prestige_title_name prestige_0");
	}*/
