...
void CPythonShop::BuildPrivateShop(const char * c_szName,DWORD days, BYTE c_shape, BYTE c_npc)
#else
...
#endif
{
	...
	#ifdef ENABLE_OFFLINE_SHOP
	CPythonNetworkStream::Instance().SendBuildPrivateShopPacket(c_szName, ItemStock,days, c_shape, c_npc);
	#else
...
PyObject * shopBuildPrivateShop(PyObject * poSelf, PyObject * poArgs)
{
	...
	if (!PyTuple_GetInteger(poArgs, 1, &days))
		return Py_BuildException();
	BYTE bShape;
	if (!PyTuple_GetByte(poArgs, 1, &bShape))
		return Py_BuildException();

	BYTE bNPC;
	if (!PyTuple_GetByte(poArgs, 2, &bNPC))
		return Py_BuildException();
	CPythonShop::Instance().BuildPrivateShop(szName,days, bShape, bNPC);
	#else
