...
void SNetworkActorData::__copy__(const SNetworkActorData& src)
{
	...
	for (int i = 0; i < CHR_SHINING_NUM; i++)
		m_dwShining[i] = src.m_dwShining[i];
#endif	
	...
	strncpy(m_szCountry, src.m_szCountry, sizeof(m_szCountry));
...
void CNetworkActorManager::__RemoveDynamicActors()
{
	...
	for( int i = 0; i < dwCharacterVIDList.size(); ++i )
	{
		...
		if( rkActorEach->IsPC() || rkActorEach->IsNPC() || rkActorEach->IsEnemy() || rkActorEach->IsBoss() )
		{
...
CInstanceBase* CNetworkActorManager::__AppendCharacterManagerActor(SNetworkActorData& rkNetActorData)
{
	...
	strncpy(kCreateData.m_szCountry, rkNetActorData.m_szCountry, sizeof(kCreateData.m_szCountry));
...
#ifdef ENABLE_SHINING_SYSTEM
	for (int i = 0; i < CHR_SHINING_NUM; i++)
		kCreateData.m_dwShining[i] = rkNetActorData.m_dwShining[i];
#endif
#ifdef ENABLE_SASH_SYSTEM
...
void CNetworkActorManager::UpdateActor(const SNetworkUpdateActorData& c_rkNetUpdateActorData)
{
	...
	if (pkInstFind)
	{
		...
		pkInstFind->ChangeCountry(c_rkNetUpdateActorData.m_szCountry);
	...
	strncpy(rkNetActorData.m_szCountry, c_rkNetUpdateActorData.m_szCountry, sizeof(rkNetActorData.m_szCountry));
