...
void CPythonPlayer::PickCloseItemVector()
{
	...
	pkInstMain->NEW_GetPixelPosition(&kPPosMain);
	/*if (!pkInstMain->IsAttacking())
		pkInstMain->GetGraphicThingInstancePtr()->InterceptOnceMotion(CRaceMotionData::NAME_PICK_UP); */ //анимация наклона за вещами
	std::vector<DWORD> itemVidList;
...
void CPythonPlayer::OpenCharacterMenu(DWORD dwVictimActorID)
{
	...
	if (pkInstMain->IsCombatZoneMap() || pkInstTarget->IsCombatZoneMap())
		return;
#endif
