...
{
	return Py_BuildValue("s", LocaleService_GetMyLang());
}

PyObject* appSetMyLang(PyObject* poSelf, PyObject* poArgs)
{
	char* szLang;
	if (!PyTuple_GetString(poArgs, 0, &szLang))
		return Py_BuildException();
...
void initapp()
{
...
#ifdef ENABLE_WINTER
	PyModule_AddIntConstant(poModule, "WINTER_MODE", 1);
#else
	PyModule_AddIntConstant(poModule, "WINTER_MODE", 0);
#endif
	...
	PyModule_AddIntConstant(poModule, "SW_ITEM_SLOT_EFFECT", 1);
#else
	PyModule_AddIntConstant(poModule, "SW_ITEM_SLOT_EFFECT", 0);
#endif
...
PyModule_AddIntConstant(poModule, "ENABLE_KEYTO_CHARACTER",	1);
