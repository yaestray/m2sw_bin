...
class CPlayerSettingsModule
{
	...
	public:
		...
		const static bool LoadGeneralMotionWolfman(char stFolder[FILE_MAX_NUM]);
		const static bool LoadGeneralMotionElfe(char stFolder[FILE_MAX_NUM]);
		...
		const static bool LoadGameElfe(DWORD dwRace, char stFolder[FILE_MAX_NUM]);
