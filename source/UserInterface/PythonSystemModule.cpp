...
PyObject * systemSetHideShining(PyObject * poSelf, PyObject * poArgs)
{
	int iFlag;
	if (!PyTuple_GetInteger(poArgs, 0, &iFlag))
		return Py_BuildException();

	CPythonSystem::Instance().SetHideShining(iFlag);

	return Py_BuildNone();
}
PyObject * systemIsHideShining(PyObject * poSelf, PyObject * poArgs)
{
	return Py_BuildValue("i", CPythonSystem::Instance().IsHideShining());
}
...
#endif
		{ "SetHideShining",				systemSetHideShining,				METH_VARARGS },
		{ "IsHideShining",					systemIsHideShining,				METH_VARARGS },
