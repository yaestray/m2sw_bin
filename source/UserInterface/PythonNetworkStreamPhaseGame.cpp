...
bool CPythonNetworkStream::RecvMaintenance()
{
	TPacketGCMaintenance packet;
	if (!Recv(sizeof(packet), &packet))
		return false;

	PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_Maintenance",
		Py_BuildValue("(ii)", packet.seconds, packet.duration));
...
void CPythonNetworkStream::GamePhase()
{
    ...
    while (ret)
	{
		...
		switch (header)
		{
			...
			case HEADER_GC_DRAGON_SOUL_CHANGE_ATTR:
				ret = RecvDragonSoulAttrChange();
				break;
#endif			
		...
		case CHAT_TYPE_WHISPER:	// 서버와는 연동되지 않는 Only Client Enum
			{
				...
				else
				{
					...
					{
						if (false == pkInstChatter->IsNPC() && false == pkInstChatter->IsEnemy() && false == pkInstChatter->IsBoss())
						{
...
bool CPythonNetworkStream::RecvTargetInfoPacket()
{
	...
	if (pInstPlayer && pInstTarget)
	{
		if (!pInstTarget->IsDead())
		{
			if (pInstTarget->IsEnemy() || pInstTarget->IsStone() || pInstTarget->IsBoss())
			{
...
bool CPythonNetworkStream::SendBuildPrivateShopPacket(const char * c_szName, const std::vector<TShopItemTable> & c_rSellingItemStock,DWORD days, BYTE bShape, BYTE bNPC)
#else
...
bool CPythonNetworkStream::RecvShopSignPacket()
{
	...
	else
	{
		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], 
			...
			Py_BuildValue("(isi)", p.dwVID, p.szSign, p.bShape)
		);
...
bool CPythonNetworkStream::SendRefinePacket(int byPos, BYTE byType)
{
...
bool CPythonNetworkStream::SendClientVersionPacket()
{
	...
	if (LocaleService_IsEUROPE() && false == LocaleService_IsYMIR())
	{
		...
		strncpy(kVersionPacket.timestamp, "1215955227", sizeof(kVersionPacket.timestamp)-1); // # python time.time 앞자리
...
bool CPythonNetworkStream::SendChangeLanguage(const char * lang)
{
	TPacketCGChangeLang pack;

	pack.header = HEADER_CG_CHANGE_LANGUAGE;
	strcpy(pack.lang, lang);
	if (!Send(sizeof(pack), &pack))
	{
		return false;
	}
