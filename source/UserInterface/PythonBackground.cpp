...
{
#ifdef ENABLE_LOAD_PROPERTY_XML
	m_PropertyManager.Initialize("data/maps/property.xml");
#else
...
void CPythonBackground::CreateSpecialShopPos(DWORD dwID, float fx, float fy, float fz, const char * c_szFileName)
{
	CMapOutdoor& rkMap = GetMapOutdoorRef();
	rkMap.SpecialEffectShopPos_Create(dwID, fx, fy, fz, c_szFileName);
}

void CPythonBackground::DeleteSpecialShopPos(DWORD dwID)
{
	CMapOutdoor& rkMap = GetMapOutdoorRef();
