...
class CMainPacketHeaderMap : public CNetworkPacketHeaderMap
{
	...
	public:
		CMainPacketHeaderMap()
		{
			...
			Set(HEADER_GC_LOGIN_SUCCESS,		CNetworkPacketHeaderMap::TPacketType(sizeof(TPacketGCLoginSuccess3), STATIC_SIZE_PACKET));
			...
			Set(HEADER_GC_DRAGON_SOUL_CHANGE_ATTR,	CNetworkPacketHeaderMap::TPacketType(sizeof(TPacketGCDragonSoulChangeAttr), STATIC_SIZE_PACKET));
#endif
			...
			Set(HEADER_GC_MAINTENANCE, CNetworkPacketHeaderMap::TPacketType(sizeof(TPacketGCMaintenance), STATIC_SIZE_PACKET));
#endif
...
#ifdef ENABLE_DS_CHANGE_ATTR
bool CPythonNetworkStream::RecvDragonSoulAttrChange()
{
		TPacketGCDragonSoulChangeAttr p;
		
		if (!Recv(sizeof(TPacketGCDragonSoulChangeAttr), &p))
		{
			Tracen("Recv p Packet Error");
			return false;
		}
	
	PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "DragonSoulAttrChange", Py_BuildValue("()"));
