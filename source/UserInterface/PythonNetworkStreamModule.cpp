...
{
	char* langStr;
	if (!PyTuple_GetString(poArgs, 0, &langStr))
		return Py_BuildException();

	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
	rkNetStream.SendChangeLanguage(langStr);
...
PyObject* netSendAnalysisMetinPacket(PyObject* poSelf, PyObject* poArgs)
{
	long long iPos;
	if (!PyTuple_GetLongLong(poArgs, 0, &iPos))
		return Py_BuildException();
	long long iCount;
	if (!PyTuple_GetLongLong(poArgs, 1, &iCount))
		return Py_BuildException();

	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
	rkNetStream.SendAnalysisMetinPacket(iPos, iCount);
	return Py_BuildNone();
}
...
PyObject* netSendShopSellPacketNew(PyObject* poSelf, PyObject* poArgs)
{
	...
	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
	rkNetStream.SendShopSellPacketNew(iPos, iCount);
	return Py_BuildNone();
}

PyObject* netSendGayaPacket(PyObject* poSelf, PyObject* poArgs)
{
	long long iPos;
	if (!PyTuple_GetLongLong(poArgs, 0, &iPos))
		return Py_BuildException();
	...
	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
	rkNetStream.SendGayaPacket(iPos, iCount);
	return Py_BuildNone();
}
...
void initnet()
{
	static PyMethodDef s_methods[] =
	{
		...
		{ "SendAnalysisMetinPacket",			netSendAnalysisMetinPacket,				METH_VARARGS },
...
#endif
		/* SunWay */
		{ "SendGayaPacket",				netSendGayaPacket,				METH_VARARGS },
		/* END */
#ifdef ENABLE_SHOW_CHEST_DROP
