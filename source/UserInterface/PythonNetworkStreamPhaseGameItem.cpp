...
bool CPythonNetworkStream::SendShopSellPacketNew(long long bySlot, long long byCount)
{
	if (!__CanActMainInstance())
	{
		return true;
	...
	TPacketCGShopBuyNew PacketShopBuyNew;
	PacketShopBuyNew.header = HEADER_CG_SHOP_BUY_NEW;
	PacketShopBuyNew.pos = bySlot;
	PacketShopBuyNew.count = byCount;

	if (!Send (sizeof (TPacketCGShopBuyNew), &PacketShopBuyNew))
	{
		Tracef ("SendPrivateShopSerchBuyItem Error\n");
		return false;
	}

	Tracef ("SendPrivateShopSerchBuyItem: ShopVid: %d  Pos %d", bySlot, byCount);

	return true;
}

bool CPythonNetworkStream::SendAnalysisMetinPacket(long long bySlot, long long byCount)
{
	if (!__CanActMainInstance())
	{
		return true;
	...
	TPacketCGAnalysisMetin PacketAnalysisMetin;
	PacketAnalysisMetin.header = HEADER_CG_ANALYSIS_METIN;
	PacketAnalysisMetin.pos = bySlot;
	PacketAnalysisMetin.count = byCount;

	if (!Send (sizeof (TPacketCGAnalysisMetin), &PacketAnalysisMetin))
	{
		Tracef ("SendPrivateShopSerchBuyItem Error\n");
		return false;
	}
...
/*SunWay */
bool CPythonNetworkStream::SendGayaPacket(long long bySlot, long long byCount)
{
	if (!__CanActMainInstance())
	{
		return true;
	...
	TPacketCGGayaSell PacketGayaSell;
	PacketGayaSell.header = HEADER_CG_GAYA_SELL;
	PacketGayaSell.pos = bySlot;
	PacketGayaSell.count = byCount;

	if (!Send (sizeof (TPacketCGGayaSell), &PacketGayaSell))
	{
		Tracef ("SendPrivateShopSerchBuyItem Error\n");
		return false;
	}
