...
class CPythonNetworkStream : public CNetworkStream, public CSingleton<CPythonNetworkStream>
{
	...
	public:
		...
		bool SendShopSellPacketNew(long long bySlot, long long byCount);
		bool SendAnalysisMetinPacket(long long bySlot, long long byCount);
		...
		bool SendBuildPrivateShopPacket(const char * c_szName, const std::vector<TShopItemTable> & c_rSellingItemStock,DWORD days, BYTE bShape, BYTE bNPC);
		#else
		bool SendBuildPrivateShopPacket(const char * c_szName, const std::vector<TShopItemTable> & c_rSellingItemStock);
		#endif		

		// Refine
		bool SendRefinePacket(int byPos, BYTE byType);
...
#endif
		/* SunWay */
		bool SendGayaPacket(long long bySlot, long long byCount);
		/* END */
