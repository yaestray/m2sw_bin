...
{	
	...
	public:
		...
		enum
		{
			...
			AFFECT_GERMANY,				// 40
			AFFECT_BLEEDING,				// 41
			AFFECT_RED_POSSESION,				// 42
			AFFECT_BLUE_POSSESION,				// 43
			AFFECT_THIRDNORMALSKILL,
			AFFECT_SOUL_BUFF,
			AFFECT_KAMIHA,
		...
		enum
		{
			...
			NEW_AFFECT_POTION_1 = 303,
			NEW_AFFECT_POTION_2 = 304,
			NEW_AFFECT_POTION_3 = 305,
			NEW_AFFECT_POTION_4 = 306,
			NEW_AFFECT_POTION_5 = 307,
	...
	public:
		...
		void					UpdateTextTailCountryFlag(const char* c_szCountry);
#ifdef ENABLE_SHINING_SYSTEM
		...
		void					ChangeCountry(const char* c_szCountry);
		DWORD					GetWeaponType();
#ifdef ENABLE_SKILL_COLOR_SYSTEM
	...
	protected:		
		...
		bool	ClearShiningEffectWeapon();
		bool	AttachShiningEffect(DWORD dwWeapon);
		bool	AttachShiningEffectWeapon(DWORD dwWeapon);
#endif		
...
#ifdef ENABLE_SHINING_SYSTEM
		DWORD					m_armorShining[2];
		DWORD					m_swordShiningRight[3];
		DWORD					m_swordShiningLeft[3];
...
inline int RaceToJob(int race)
{
	...
	return race % JOB_NUM;
	*/
	if (race == 0 || race == 4) //warrior
		return 0;
	else if (race == 1 || race == 5) //ninja
		return 1;
	else if (race == 2 || race == 6) //sura
		return 2;
	else if (race == 3 || race == 7) //shaman
		return 3;
}
