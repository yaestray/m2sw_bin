...
DWORD CPythonPlayer::__GetTotalAtk(DWORD dwWeaponPower, DWORD dwRefineBonus)
{
	...
	//dwTotalAtk = __GetLevelAtk() + (__GetStatAtk() + dwWepAtk) * __GetHitRate() / 100;
	dwTotalAtk = __GetLevelAtk() + __GetStatAtk() + dwWepAtk;
...
CPythonPlayer::CPythonPlayer(void)
{
	...
	m_kMap_dwAffectIndexToSkillIndex.insert(make_pair(int(CInstanceBase::AFFECT_THIRDNORMALSKILL), 189));
	m_kMap_dwAffectIndexToSkillIndex.insert(make_pair(int(CInstanceBase::AFFECT_SOUL_BUFF), 191));
	m_kMap_dwAffectIndexToSkillIndex.insert(make_pair(int(CInstanceBase::AFFECT_KAMIHA), 193));
