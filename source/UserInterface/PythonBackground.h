...
public:
	...
	void CreateCombatZoneTargetsLeaving(DWORD dwID, DWORD dwChrVID);
	void CreateCombatZoneTargetsLeaving(DWORD dwID, long lx, long ly);
#endif
	...
	void CreateSpecialShopPos(DWORD dwID, float fx, float fy, float fz, const char * c_szFileName);
	void DeleteSpecialShopPos(DWORD dwID);
...
private:
...
#if defined(WJ_COMBAT_ZONE)
	std::map<DWORD, DWORD> m_kMapCombatZone_dwTargetID_dwChrID;
#endif
	...
	std::map<DWORD, DWORD> m_kMapShop_dwTargetID_dwChrID;
